...
use crate::program::{ config::ENV_VARIABLES, utils::{ self, print_utils::press_to_continue } };
...
lazy_static! {
    pub static ref MENU_COMMANDS: Mutex<Vec<MenuCommand>> = {
        ...
        commands.push(MenuCommand::create_instance("Print saved versions", todo!()));
        commands.push(MenuCommand::create_instance("Print current version", todo!()));
